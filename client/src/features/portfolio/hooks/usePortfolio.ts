...
export function useExportProjectPdf(projectId: number) {
  return useMutation({
    mutationFn: async () => {
      await downloadBlob(`/exports/project/${projectId}/pdf`, `project-${projectId}-report.pdf`);
    },
  });
}
...
export function useExportProjectExcel(projectId: number) {
  return useMutation({
    mutationFn: async () => {
      await downloadBlob(`/exports/project/${projectId}/excel`, `project-${projectId}-report.xlsx`);
    },
  });
}
...
export function useExportPortfolioPdf() {
  return useMutation({
    mutationFn: async () => {
      await downloadBlob('/exports/portfolio/pdf', 'portfolio-report.pdf');
    },
  });
}
...
export function useExportPortfolioExcel() {
  return useMutation({
    mutationFn: async () => {
      await downloadBlob('/exports/portfolio/excel', 'portfolio-report.xlsx');
    },
  });
}
